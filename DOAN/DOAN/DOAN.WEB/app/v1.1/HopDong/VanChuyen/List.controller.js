sap.ui.define([
    ...
    return Controller.extend('app.HopDong.List', {
        ...
        onInit: function () {
            ...
            this.bus.subscribe('HopDongChannel', 'VanChuyenLoad', this.loadData, this);
            //this.bus.subscribe('HopDongChannel', 'reVanChuyenLoad', this.reloadData, this);
            this.bus.subscribe('HopDongChannel', 'addVatTuVC', this.addVatTuVC, this);
            this.bus.subscribe('HopDongChannel', 'onCloseVatDungVanChuyenAdd', this.onCloseVatDungVanChuyenAdd, this);
            this.loadVatDung();
        },
        ...
        loadData: function (oC, oE, oData) {
            ...
            if (oData.Id) {
                ...
                Connector.getFromApi(sdConfig.adminApiEndpoint + 'ChiTietVanChuyen/khoitao/' + oData.Id, {
                    fnProcessData: function (data) {
                        if (data && data.length > 0) {
                ...
                Connector.getFromApi(sdConfig.adminApiEndpoint + 'vanchuyen/getnewid', {
                    fnProcessData: function (data) {
                        let count = 0;
                        if (data) {
                            data = data.maPhieu.replace('VCN', '');
                            count = Number(data) +1;
                        }
                        let datax = {
                            maPhieu: 'VCN' + count,
        ...
        loadThucdon: function () {
            const root = this;
            Connector.getFromApi(sdConfig.adminApiEndpoint + 'thucdon/HopDong/' + root.mainId, {
                fnProcessData: function (data) {
                    if (data && data.length > 0) {
        ...
        reloadData: function () {
            const root = this;
            Connector.getFromApi(sdConfig.adminApiEndpoint + 'ChiTietVanChuyen/' + this.mainId, {
                fnProcessData: function (data) {
                    if (data && data.length > 0) {
                        root.formatDataVanChuyen(data);
                    }
        ...
        loadInfoDonCo: function () {
            const root = this;
            if (this.mainId) {
                Connector.getFromApi(sdConfig.adminApiEndpoint + 'HopDong/' + this.mainId, {
                    fnProcessData: function (data) {
                        if (data) {
                            data.maPhieu = 'VC' + data.id;
        ...
        addVatDung: function () {
            ...
            if (!this._VatDungAdd) {
                Fragment.load({
                    ...
                    root.bus.publish('HopDongChannel', 'loadAddVatDungPage', { arrID: arrId });
                });
            }
            else {
                root._VatDungAdd.open();
                this.bus.publish('HopDongChannel', 'loadAddVatDungPage', { arrID: arrId });
            }
        },
        ...
        onAddButtonPress: function () {
            const root = this;
            let x = this.mainModel.getData();
            x.ngayTao = x.ngayTao == moment().format('DD/MM/YYYY') ? moment().format('YYYY-MM-DD') : x.ngayTao
            let obj = {
                    ...
                    if (data) {
                        ...
                        Connector.postToApi(sdConfig.adminApiEndpoint + "chitietvanchuyen", {
                            ...
                            fnSuccess: function (data) {
                                ...
                                //root.reLoadData();
                                this.mainId = null;
                                root.bus.publish('HopDongChannel', 'onCloseVanChuyenAdd');
                                root.bus.publish('VanChuyenChannel', 'reLoadData');
