sap.ui.define([
    ...
    'use strict';
    return Controller.extend('app.MonAn.List', {
        ...
        onInit: function () {
            ...
            this.loadData()
            this.bus.subscribe('MonAnChannel', 'switchToDetailPage', this.switchToDetailPage, this);
            this.bus.subscribe('MonAnChannel', 'onCloseMonAnView', this.onCloseMonAnView, this);
            this.bus.subscribe('MonAnChannel', 'onCloseMonAnAdd', this.onCloseMonAnAdd, this);
            ...
            this.bus.subscribe('MonAnChannel.Detail', 'reLoadData', this.reLoadData, this);
        },
        loadData: function () {
            const root = this;
        ...
        updateStatusItems: function (oEvent) {
            ...
            if (!root.fragUpdateStatus) {
                Fragment.load({
                    name: "app.MonAn.fragment.UpdateStatus",
                    type: "XML",
                    controller: root
                }).then(function (frag) {
        ...
        btnUpdateStatus: function (oEvent) {
            ...
            if (selectedItem) {
                ...
                if (indices.length > 0) {
                    ...
                    Connector.postToApi(sdConfig.adminApiEndpoint + 'MonAn/updateStatus', {
                        oParameters: {
                            ids: listId,
                            status: sts,
                        },
        ...
        searchFilter: function () {
            ...
            Connector.postToApi(sdConfig.adminApiEndpoint + 'MonAn/filter', {
        ...
        onCellClick: function (oEvent) {
            ...
            if (selected)
                this.bus.publish('MonAnChannel', 'switchToDetailPage', { Id: selected.id, title: selected.tenMonAn });
        },
        ...
        switchToDetailPage: function (oChannel, oEvent, oData) {
            const root = this;
            if (!this._MonAnDetail) {
                Fragment.load({
                    id: root.getView().getId(),
                    name: "app.MonAn.Detail",
                    type: "XML",
                    controller: this
                }).then(function (frag) {
                    root._MonAnDetail = frag;
                    root._MonAnDetail.open();
                    root.bus.publish('MonAnChannel', 'loadDetailPage', { Id: oData.Id, title: oData.title });
                });
            }
            else {
                root._MonAnDetail.open();
        ...
        onCloseMonAnView: function () {
            this._MonAnDetail.close();
        },
        ...
        onCloseMonAnAdd: function () {
            this._MonAnAdd.close();
        },
        ...
        onRowDelete: function (oEvent) {
            const root = this;
            let selected = this.getView().getModel('mainModel').getProperty('', oEvent.getParameter('row').getBindingContext('mainModel'));
            MessageBox.show('Bạn muốn xóa ' + selected.tenMonAn, {
                icon: MessageBox.Icon.WARNING,
                title: 'Xác nhận',
                actions: [MessageBox.Action.DELETE, MessageBox.Action.NO],
                onClose: function (oAction) {
                    if (oAction == MessageBox.Action.DELETE) {
                        Connector.deleteToApi(sdConfig.adminApiEndpoint + 'MonAn/' + selected.id, {
                            fnSuccess: function (data) {
                                root.reLoadData();
        ...
        deleteItems: function (oEvent) {
            ...
            if (indices.length > 0) {
                ...
                MessageBox.show('Xóa tất cả các mục được chọn?', {
                    ...
                    onClose: function (oAction) {
                        if (oAction == MessageBox.Action.DELETE) {
                            Connector.deleteToApi(sdConfig.adminApiEndpoint + 'MonAn/list', {
                                oParameters: listId,
                                fnSuccess: function (data) {
                                    root.reLoadData();
                                    MessageToast.show("Xóa thành công!", { width: '25em', duration: 5000 });
        ...
        onRowEdit: function (oEvent) {
            ...
            if (!this._MonAnEdit) {
                Fragment.load({
                    ...
                    root._MonAnEdit = frag;
                    root._MonAnEdit.open();
                    root.bus.publish('MonAnChannel', 'loadEditPage', { Id: selected.id, title: selected.tenMonAn });
                });
            }
            else {
                root._MonAnEdit.open();
