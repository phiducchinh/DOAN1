sap.ui.define([
    ...
    const oController = {
        ...
        closeAreaTD: function () {
            this.clearForm();
            this.bus.publish('HopDongChannel', "onCloseThucDonEdit");
        },
        saveTD: async function () {
            ...
            if (listThucDon && listThucDon.length <= 1) {
                ...
                let newlistThucDon = listThucDon.map((item) => {
                    return {
                        idHopDong: root.idHopDong,
                        idMonAn: item.idMonAn,
                        giaTien: Number(item.giaTien),
                    }
                });
                Connector.postToApi(sdConfig.adminApiEndpoint + 'thucdon/editList', {
                    oParameters: newlistThucDon,
                    fnSuccess: function (data) {
                        root.closeAreaTD();
                        root.bus.publish('HopDongChannel', 'loadThucDonData');
                        MessageToast.show("Thêm thành công", { width: '25em', duration: 5000 });
                    }
                })
            }
    ...
    return Controller.extend('app.HopDong.ThucDon.Edit', oController);
});